...
class RegistrationView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            return Response('Аккаунт успешно зарегистрирован', status=status.HTTP_200_OK)
...
class LogoutView(APIView):
    permission_classes = [IsAuthenticated] #проверка прав пользователя на какое-то действие

    def post(self, request):
        user = request.user #получаем юзера
